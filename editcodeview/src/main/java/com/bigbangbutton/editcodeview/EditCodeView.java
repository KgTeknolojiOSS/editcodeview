...
public class EditCodeView extends View
    ...
    private static final char DEFAULT_CODE_MASK = '*';
    private static final char DEFAULT_PLACE_HOLDER_MASK = '\u2022';
    ...
    private char codeHiddenMask;
    private char placeHolderMask;
    ...
    private void initDefaultAttrs(Context context) {
        ...
        placeHolderMask = DEFAULT_PLACE_HOLDER_MASK;
    }
    ...
    private void initCustomAttrs(Context context, AttributeSet attributeSet) {
        ...
        fontStyle = attributes.getInt(
                R.styleable.EditCodeView_ecv_fontStyle, fontStyle);
        ...
        if (!TextUtils.isEmpty(mask)) {
            codeHiddenMask = mask.charAt(0);
        }

        placeHolderMode = attributes.getBoolean(
                R.styleable.EditCodeView_placeHolderMode, placeHolderMode);

        String placeHolder = attributes.getString(R.styleable.EditCodeView_placeHolderMask);
        if (!TextUtils.isEmpty(placeHolder)) {
            placeHolderMask = placeHolder.charAt(0);
        }

        String textFontTypeTemp = attributes.getString(R.styleable.EditCodeView_textFontType);
        if (!TextUtils.isEmpty(textFontTypeTemp)) {
            textFontType = textFontTypeTemp;
        }
    ...
    private void initPaints(Context context) {
        ...
        if (!TextUtils.isEmpty(textFontType)){
            Typeface customFont = Typeface.createFromAsset(context.getAssets(), textFontType);
    ...
    private void drawText(Canvas canvas) {
        int length = placeHolderMode ? codeLength : editable.length();
        for (int i = 0; i < length; i++) {
            boolean isText = i < editable.length();
            char symbol = !isText ? placeHolderMask : (codeHiddenMode ? codeHiddenMask : editable.charAt(i));
            float textPosX = sectionWidth * i + sectionWidth / 2 - symbolMaskedWidth / 2;
            textPaint.setColor(isText ? textColor : (isSelected ? underlineSelectedColor : underlineBaseColor));
            canvas.drawText(new char[]{symbol}, 0, 1, textPosX, textPosY, textPaint);
        }
    }
    ...
    private void measureSizes(int viewWidth, int viewHeight) {
        ...
        symbolMaskedWidth = textPaint.measureText(String.valueOf(codeHiddenMask));
